...
http.listen(3000, function () {
	...
	mongoClient.connect("mongodb://localhost:27017", function (error, client) {
		...
		app.get("/logout", function (request, result) {
			result.redirect("/");//chaaaaaaaaaaaaaaange
		});
		...
		/*app.get('/friends', async (request, result) => {
			try {
			  const userId = request.fields.query;
		  
			  const currentUser = await database.collection("users").findOne({ _id: userId });
			  const friendIds = currentUser.friends; // Assuming currentUser.friends is an array of friend IDs
			  
			  // Récupérer les détails des amis de l'utilisateur
			  const friends = await database.collection("users").find({ _id: { $in: friendIds } }).toArray();
			
			  result.render('friends', { currentUser, friends });
			} catch (error) {
			  console.error(error);
			  result.send('Une erreur s\'est produite lors de la récupération des amis.');
			}
		  });
		  ...
		  app.get('/friends', async (request, result) => {
			try {
			  ...
			  const currentUser = await database.collection("users").findOne({ _id: userId });
			  const friendIds = currentUser.friends._id; // Assuming currentUser.friends is an array of friend IDs
			  ...
			  const cursor= collection("users").find({ _id: { $in: friendIds } });
		  ...
		  app.get('/friends/:query', async (request, result) => {
			try {
			  const userId = request.params.query;
